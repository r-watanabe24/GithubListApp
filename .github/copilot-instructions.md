...
- 修正すべき場合は、修正例を必ず示すこと
  -	すべての修正は suggestion ブロックのみで出力する。 
  - 説明と suggestion は分離し、同じブロックに混在させない。 
  - 行頭開始（列0）・前後に空行1行・フェンスに余計な空白を入れない。 
  - インラインコード（1本バッククォート）で 複数行を包まない。 
  - リスト・表・引用の内側に置かない（独立段落に置く）。 
  - 末尾は改行で終端、行末空白を残さない。 
  - 既存がタブならタブ、スペースならスペースでインデントを完全一致。
