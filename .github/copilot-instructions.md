...
- 修正すべき場合は、修正例を示すこと
### 修正提案の出力ルール（SUGGESTION_ONLY）
- すべての修正は **GitHub の suggestion ブロック**のみで出力すること。
  - コメントや説明は **suggestion と同じブロックに混在させない**（説明は別メッセージ側に書く）。
  - フェンスは **行頭（列0）から開始**し、前後に空行を1行ずつ置く。リスト・表・引用の“内側”にネストしない。
